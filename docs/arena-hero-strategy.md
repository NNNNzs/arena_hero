...
Core 只有在连续 8 个新 Tick 没有可见资源、没有任何 Worker cargo、没有可见敌人、HP 满、护盾至少 3、目标相邻格无障碍且无任何实体时，才向高价值探索边界启动普通迁移。只要 Worker 有货，迁移推迟到 Core 静止并完成存入之后。启动后不重复提交迁移动作，也不自动取消；每 Tick 显式等待直至服务器完成或报告失败。
