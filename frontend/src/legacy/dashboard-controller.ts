// The controller is intentionally kept as a typed-build compatibility bridge
// during the incremental Vue migration. Its public behavior is covered by the
// existing dashboard contract tests and will be split into composables next.
// @ts-nocheck
...
async function fetchEvents({ limit = 50, category = activeEventCategory, from_tick = null } = {}) {
  ...
  eventInFlight = true;
  try {
    const data = await dashboardApi.events({ limit, category, fromTick: from_tick });
...
async function loadEarlierReplay() {
  ...
  try {
    ...
    const data = await dashboardApi.replay({ limit: 32, toTick: earliest - 1 });
...
async function refresh() {
  ...
  try {
    var payload = await dashboardApi.dashboard();
    setLive(payload);
    await refreshPolicy();
...
async function fetchMapMemory() {
  ...
  memoryInFlight = true;
  try {
    var data = await dashboardApi.mapMemory();
...
async function refreshReplay() {
  ...
  try {
    ...
    const data = await dashboardApi.replay({ limit: 8, fromTick: lastReplayTick });
...
async function fetchReplayTimeline() {
  try {
    const data = await dashboardApi.replayTimeline(64);
...
async function fetchReplayHistory() {
  try {
    const data = await dashboardApi.replay({ limit: 32 });
    mergeReplayFrames(data.frames || []);
  } catch (_) {}
}
