...
import { KIND_LABELS, SQUAD_TYPE_ICONS, SQUAD_TYPE_LABELS, actionLabel, localizeStrategyText, statusLabel, taskLabel } from '../domain/labels'
...
<template>
  <aside class="panel unit-panel">
    ...
    <div id="tabSquadsView" v-show="activeTab === 'squads'">
      <div class="squad-toolbar"><span id="squadTotalStats" class="muted">共 {{ squads.length }} 个编组 · {{ totalMembers }} 人</span><div class="squad-toolbar-actions"><button id="btnExpandAllSquads" class="filter-btn" @click="expandAll">全部展开</button><button id="btnCollapseAllSquads" class="filter-btn" @click="collapseAll">全部折叠</button></div></div>
      <div id="squadList" class="squad-list muted">
        <div v-for="squad in squads" :key="squad.id" class="squad-card" :class="{ 'is-collapsed': collapsedSquadIds.has(squad.id) }" :data-squad-id="squad.id">
          <button class="squad-card-header" :data-squad-id="squad.id" @click="toggleSquad(squad.id)"><span class="squad-card-title"><span class="squad-collapse-indicator">{{ collapsedSquadIds.has(squad.id) ? '▶' : '▼' }}</span><strong>{{ SQUAD_TYPE_ICONS[squad.type || ''] || '🚩' }} {{ squad.name }}</strong><span class="tag">{{ squad.members?.length || 0 }} 人</span></span><span class="state-pill">{{ statusLabel(squad.status || 'UNKNOWN') }}</span></button>
          <div v-if="!collapsedSquadIds.has(squad.id)" class="squad-card-body"><div class="squad-meta"><span>编组类型</span><b>{{ SQUAD_TYPE_LABELS[squad.type || ''] || squad.type }}</b><span v-if="squad.target"> · 目标 {{ squad.target.join(',') }}</span></div><button class="squad-flow focus-squad" :data-squad-id="squad.id" @click="focusSquad(squad.id)">聚焦编组目标 · {{ localizeStrategyText(squad.causality?.flow_reason || '当前策略模式重新分配编组。') }}</button><div class="squad-members-box"><div v-for="member in squad.members || []" :key="member.alias" class="squad-member-row" :class="{ 'is-selected': member.alias === dashboard.selectedAlias.value }"><button class="squad-member-info select-squad-unit" :data-alias="member.alias" @click="memberSelected(member.alias)"><span class="unit-glyph" :class="`kind-${String(member.kind || 'worker').toLowerCase()}`"></span><b>{{ KIND_LABELS[member.kind || ''] || member.kind }}</b><small class="muted">{{ member.alias.slice(0, 15) }}</small><div class="squad-member-sub">{{ member.position ? `[${member.position.join(',')}]` : '' }} {{ member.hp == null ? '' : `HP ${member.hp}` }}{{ member.cargo ? ` · 货 ${member.cargo}` : '' }}{{ member.task ? ` · ${taskLabel(member.task)}` : '' }}</div></button><select class="squad-switch-select" :value="squad.id" :data-alias="member.alias" aria-label="调整单位编组" @click.stop @change="switchSquad(member, ($event.target as HTMLSelectElement).value)"><option v-for="targetSquad in squads" :key="targetSquad.id" :value="targetSquad.id">{{ targetSquad.name }}</option></select></div><div v-if="!squad.members?.length" class="muted">暂无分配成员</div></div></div>
        </div>
